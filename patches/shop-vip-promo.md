...
- **功能**: 定义 VIP 引导按钮的样式。
- **样式**: 
  - `.xingxy-vip-group-btn`: 金色渐变背景，Flex 权重 1.8（最宽）。
  - `.xingxy-vip-secondary-btn`: 灰色背景，Flex 权重 0.8（最窄），作为原价购买按钮。
...
- **V5 (最终版)**: 原生按钮组内的三按钮布局。保留[加入购物车]，将原[立即购买]一分为二，左侧为[VIP引导]，右侧为[原价购买(灰)]。
