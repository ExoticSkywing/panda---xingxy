...
/**
 * 自定义 Shop 移动端底部 Tabbar (V6 布局)
 * 
 * @param string $btn 原有的按钮 HTML
 * @return string 修改后的按钮 HTML
 */
function xingxy_shop_single_footer_tabbar($btn) {
    global $post;
    ...
    // V6 布局: [加购] [VIP导流] [原价购(灰)]
    // 注意：移动端底部按钮通常不在 Vue 实例内，需要使用 data-onclick 触发隐藏的 Desktop 按钮点击事件
    $pay = '
    <div class="but-group shop-paybtn-group">
        <button class="but c-yellow shop-add-cart-btn" data-onclick=".shop-add-cart-btn">加入购物车</button>
        <a href="https://xingxy.manyuzo.com/会员介绍" target="_blank" class="but xingxy-vip-group-btn">
            <i class="fa fa-diamond mr3"></i>星冕会员享半价
        </a>
        <button class="but jb-red shop-buy-btn xingxy-vip-secondary-btn" data-onclick=".shop-buy-btn">原价购买</button>
    </div>';
...
// 移除 Zibll 原生的 shop footer filter，并添加自定义的
add_action('wp_loaded', function() {
    remove_filter('footer_tabbar', 'zib_shop_single_footer_tabbar');
    add_filter('footer_tabbar', 'xingxy_shop_single_footer_tabbar');
});
